...
function ajaxCall(url, confirmMessage, onSuccess) {
    if (confirmMessage === "" || confirm(confirmMessage)) {
        $.ajax(url).done(onSuccess).fail(function () {
...
function ajaxCallWithFormData(url, formId, confirmMessage, onSuccess) {
    if (confirmMessage === "" || confirm(confirmMessage)) {
        $.ajax({url: url, data: $("#" + formId).serializeArray()}).done(onSuccess).fail(function () {
            alert("Ajax call failed!");
        });
    }
}
...
function addHrefParams(element, paramContainer) {
    $(paramContainer).find(".toggler-checkbox").each(function() {
        if ($(this).is(':checked')) {
...
jQuery(document).ready(function ($) {
    ...
    $('tr.calculates-total .switch-router-calculation-details').click(function() {
        if ($(this).val() == 'partial') {
            $(this).val('lunar');
    ...
    $('tr.calculates-total .switch-router-calculation-part-details').click(function() {
        if ($(this).val() == 'lung') {
            $(this).val('scurt');
    ...
    $('tr.calculates-total .calculate-product-part').blur(function() {
        let total = 1;
        $(this).closest('tr').find('.calculate-product-part').each(function() {
    ...
    $('tr.calculates-total .data_fact_until').find('input.flatpickr,input.flatpickr-input').change(function() {
        setDescription($(this));
        updateCalculationDetails($(this));
        $(this).closest('tr').find('.calculate-product-part').first().blur();
    });
    ...
    $('#iservice-body input[data-required-minimum].agressive, #iservice-body [data-required-minimum].agressive input').blur(function () {
    ...
    $('#iservice-body input[data-estimated]').blur(function () {
    ...
    $('#iservice-body .iservice-form .required').blur(function () {
        if ($(this).val() === '') {
    ...
    $('#iservice-body .iservice-form').submit(function (event) {
    ...
    $('#iservice-body .iservice-form .submit').click(function (event) {
        var message = '';
        ...
        if ($(this).attr('data-required') !== undefined) {
            $.each($(this).attr('data-required').split(','), function () {
                $to_check = $("[name='" + this + "']");
        ...
        $('#iservice-body input[data-required-minimum], #iservice-body [data-required-minimum] input').each(function() {
            let label = '';
            let required_minimum = 0;
            let value_to_compare = 0;
            let calling_component = null;
        ...
        $('#iservice-body input[data-estimated]').each(function() {
            if (accepted_estimates[this.name]) {
                return;
            }
            estimated = parseInt($(this).attr("data-estimated"));
    ...
    var updateTextarea = function ($textarea, $changeCheckboxes) {
        ...
        $textarea.closest('form').find('.textarea-checkboxes').each(function () {
        ...
        $relatedCheckboxDiv.find('input.checkbox').each(function () {
            if ($changeCheckboxes) {
    ...
    $('#iservice-body .iservice-form [data-target-textarea] .checkbox').click(function () {
        updateTextarea($("." + $(this).closest('.textarea-checkboxes').attr('data-target-textarea')));
    });

    $('#iservice-body .iservice-form .change-cartridge-description').change(function () {
        updateTextarea($(this), true);
    });

    $('#iservice-body .iservice-form .change-cartridge-description').each(function () {
        $(this).change();
        //updateTextarea($(this), true);
    });

    $("[data-for='_operator_reading']").click(function () {
    ...
    $("#iservice-body .iservice-form.two-column .ticket-followups tr.tab_bg_3").each(function () {
        //$(this).find("td.center").css("width","10em");
        $(this).find("td.b").removeAttr("width");
    });

    $("[name=generate_magic_link].new").click(function () {
    ...
    $("li.collapsible label").click(function() {
        $(this).siblings("ul").toggle();
    });

    var above_actions = false;
    var above_actions_menu = false;
    $(".actions.collapsible").each(function () {
    ...
    $(".actions-menu img").click(function () {
    ...
    $(".has-bootstrap-tooltip").each(function() {
        $(this).tooltip({html: true, trigger: 'click'});
    });

    $(".checkbox-helper").change(function () {
        $(getEscapedId($(this).data("for"))).val($(this).is(':checked') ? 1 : 0);
    });

    $(".compare-text-tooltip").each(function () {
