...
BEGIN
    ...
    WHERE c1.id = cartridgeId
      ...
      AND FIND_IN_SET (cfc2.suppliers_id_field, cfs.groupfield)
...
BEGIN
    ...
    LEFT JOIN glpi_plugin_fields_suppliercustomfields cfs on cfs.items_id = cfc.suppliers_id_field and cfs.itemtype = 'Supplier'
...
BEGIN
    ...
    LEFT JOIN glpi_plugin_fields_printercustomfields cfp ON cfp.itemtype = 'Printer' AND cfp.items_id = c.printers_id
    LEFT JOIN glpi_tickets t on t.id = cfc.tickets_id_use_field
...
BEGIN
    ...
    SELECT 100 *(addition + 1 - (getPrinterCounterEstimate(c.printers_id, color) - installedCounter) / cfci.atc_field)
    INTO percentage
    FROM glpi_cartridges c
...
this_proc: BEGIN
  ...
  SELECT
      COUNT(t.id)
    , MIN(CASE color WHEN 1 THEN cft.total2_color_field ELSE cft.total2_black_field + cft.total2_color_field END)
    , MAX(CASE color WHEN 1 THEN cft.total2_color_field ELSE cft.total2_black_field + cft.total2_black_field END)
    , MIN(cft.effective_date_field)
    , MAX(cft.effective_date_field)
  INTO
      ticketCount
    , minCounter
    , maxCounter
  ...
  IF DATEDIFF(maxDataLuc, minDataLuc) < minDayCount THEN
    ...
    ELSE
      ...
      SELECT MIN(CASE color WHEN 1 THEN cft.total2_color_field ELSE t.total2 END), MAX(CASE color WHEN 1 THEN cft.total2_color_field ELSE t.total2 END), MIN(cft.effective_date_field), MAX(cft.effective_date_field)
      INTO minCounter, maxCounter, minDataLuc, maxDataLuc
      FROM (
        SELECT cft.total2_black_field + cft.total2_color_field total2, cft.total2_color_field, cft.effective_date_field
        FROM glpi_tickets t
        JOIN glpi_plugin_fields_ticketticketcustomfields cft on cft.items_id = t.id and cft.itemtype = 'Ticket'
        JOIN glpi_items_tickets it ON it.tickets_id = t.id AND it.itemtype = 'Printer' AND it.items_id = printerId
        WHERE t.is_deleted = 0 AND t.`status` = 6
        ORDER BY cft.effective_date_field DESC
        LIMIT ticketCount
      ) t;
  ...
  JOIN glpi_items_tickets it ON it.tickets_id = t.id AND it.itemtype = 'Printer' AND it.items_id = printerId
  WHERE t.is_deleted = 0 AND t.`status` = 6
    AND DATEDIFF(maxDataLuc, cft.effective_date_field) < minDayCount;
