...
class Printers extends View
{
    ...
    protected function getSettings(): array
    {
        ...
        if (!empty($import)) {
            ...
            $emaintenance_query   = "select " . PluginIservicePrinter::getSerialFieldForEM() . " id, pc.id customfield_id from glpi_plugin_fields_printerprintercustomfields pc join glpi_printers p on p.id = pc.items_id and pc.itemtype='Printer' where pc.em_field = 1";
            foreach (IserviceToolBox::getQueryResult($emaintenance_query) as $id => $printer_customfield) {
        ...
        return [
            ...
            'query' => "
                        ...
                        WHERE p.is_deleted = 0 {$this->getRightCondition()} $contract_condition
                            ...
                            AND ((cfp.week_nr_field is null AND '[week_nr_field]' = '%%') OR cfp.week_nr_field LIKE '[week_nr_field]')
                            [tech_id]
                            [supplier_id]
                            [em_disabled]
                        GROUP BY p.id
            ...
            'filters' => [
                ...
                'em_disabled' => [
                    ...
                    'format' => 'AND cfp.disable_em_field = 1',
                    'header' => 'printer_name',
