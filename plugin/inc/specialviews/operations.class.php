            ...
            'query' => "
                SELECT * FROM (
                    SELECT
                        ...
                        , cft.total2_black_field + coalesce(cft.total2_color_field, 0) ticket_counter_total
                        , cft.total2_black_field ticket_counter_black
                        , cft.total2_color_field ticket_counter_color
                        , cft.contact_name_field ticket_contact
                        , cft.contact_phone_field ticket_contact_num
                        , cft.device_observations_field ticket_comment
                        /*, cft.status_aparat ticket_printer_status*/
                        , cft.without_paper_field ticket_without_papers
                        , cft.exported_field ticket_exported
                        , cft.plugin_fields_ticketexporttypedropdowns_id
                    ...
                    LEFT JOIN ( SELECT cat.tickets_id, GROUP_CONCAT(CONCAT(ci.ref, '&nbsp;<span title=\"', IF(cfc.plugin_fields_cartridgeitemtypedropdowns_id IN (2,3,4), cft.total2_color_field, cft.total2_black_field + coalesce(cft.total2_color_field, 0)), ' + (', cfc.atc_field, ' * ', cfc.life_coefficient_field, ')', '\">(pana&nbsp;', IF(cfc.plugin_fields_cartridgeitemtypedropdowns_id IN (2,3,4), cft.total2_color_field, cft.total2_black_field + coalesce(cft.total2_color_field, 0)) + ROUND(cfc.atc_field * cfc.life_coefficient_field), ')</span><br>[', c.id,COALESCE(CONCAT(' -> ',cat.cartridges_id_emptied),' -> <span style=\"color:red;\" title=\"nu golește nimic\">!!!</span>'),']') SEPARATOR '<br>') cartridges
                                ...
                                LEFT JOIN glpi_plugin_fields_cartridgeitemcartridgeitemcustomfields cfc on cfc.items_id = ci.id and cfc.itemtype = 'CartridgeItem'
                    ...
                    WHERE t.is_deleted = 0
                        ...
                        AND (cft.effective_date_field IS NULL OR cft.effective_date_field <= '[data_luc]')
                        [printer_id]
                    GROUP BY t.id
                ) t
                WHERE 1=1
