        ...
        return [
            ...
            'query' => "
                        SELECT
                            ...
                            , cft.plugin_fields_ticketexporttypedropdowns_id ticket_export_type
                            , cft.exported_field ticket_exported
                            , p.id printer_id
                            , CONCAT(p.name, CASE WHEN cfp.em_field = 1 THEN ' [EM]' ELSE '' END, CASE WHEN scf.cm_field = 1 THEN ' [CM]' ELSE '' END) printer_name
                            , cfp.usage_address_field
                            , l.completename printer_location
                            , s.id supplier_id
                            , s.name supplier_name
                            , t.date date_open
                            , CASE cft.effective_date_field WHEN '0000-00-00 00:00:00' THEN NULL WHEN '0000-00-00' THEN NULL ELSE cft.effective_date_field END effective_date
                            , u.id tech_park_id
                        ...
                        WHERE t.is_deleted = 0 $right_condition
                            ...
                            AND ((cfp.usage_address_field is null AND '[usage_address_field]' = '%%') OR cfp.usage_address_field LIKE '[usage_address_field]')
                            ...
                            AND (cft.effective_date_field IS NULL OR cft.effective_date_field <= '[effective_date]')
            ...
            'filters' => [
                ...
                'effective_date_start' => [
                    ...
                    'caption' => '',
                    'format' => "AND ((cft.effective_date_field IS NULL AND '%1\$s' = '') OR cft.effective_date_field >= '%1\$s 00:00:00')",
                    'style' => 'width: 6em;',
