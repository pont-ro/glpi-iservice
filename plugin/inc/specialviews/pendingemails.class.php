...
use \PluginIservicePendingEmail;
use \PluginIservicePendingEmailUpdater;
use \GLPIMailer;

class PendingEmails extends View
{

    public static $rightname = 'plugin_iservice_view_pending_emails';

    public static $icon = 'ti ti-mail-exclamation';
    ...
    public static function getBodyDisplay($row_data): string
    {
        global $CFG_PLUGIN_ISERVICE;
        $ajax_call = "ajaxCall(\"$CFG_PLUGIN_ISERVICE[root_doc]/ajax/managePendingEmail.php?operation=update_body&id=$row_data[id]&body=\" + encodeURIComponent($(\"#body-edit-$row_data[id]\").val()), \"\", function(message) {if(message !== \"" . IserviceToolBox::RESPONSE_OK . "\") {alert(message);} else {\$(\"#body-edit-span-$row_data[id]\").toggle(); \$(\"#body-image-$row_data[id]\").attr(\"title\", $(\"#body-edit-$row_data[id]\").val());}});";
    ...
    {
        global $CFG_PLUGIN_ISERVICE;
        if (IserviceToolBox::getInputVariable('mass_action_send_email')) {
            $result = [
                'ignored' => 0,
                'error'   => 0,
                'sent'    => 0,
            ];
            foreach (array_keys(IserviceToolBox::getArrayInputVariable('item', [])['pending_emails'] ?? []) as $item) {
                global $CFG_GLPI;
        ...
        return [
            ...
            'query' => "
                ...
                left join glpi_plugin_iservice_suppliers s on s.id = ic.suppliers_id 
                ",
            'mass_actions' => [
                'send_email' => [
