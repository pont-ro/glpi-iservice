        ...
        if (null !== ($accessible_printer_ids = \PluginIservicePrinter::getAccessibleIds())) {
            ...
            $restriction_filter = "
                AND c.id IN (SELECT c1.id
                         ...
                         JOIN glpi_plugin_fields_cartridgecartridgecustomfields cfc on cfc.items_id = c1.id and cfc.itemtype = 'Cartridge'
        ...
        $settings = [
            ...
            'query' => "
                        SELECT
                          ...
                          , c.date_in
                          , c.date_use
                          , cfc.tickets_id_use_field saved_installer_ticket_id
                          , c.date_out
                          , cfc.tickets_id_out_field saved_out_ticket_id
                          , cfc.pages_use_field
                          , cfc.pages_color_use_field
                          , cfc.printed_pages_field
                          , cfc.printed_pages_field_color
                          , cfc.printed_pages_field + cfc.printed_pages_field_color total_printed_pages
                          , ci.id cartridgeitem_id
                          , ci.name
                          , ci.ref
                          , cfci.mercury_code_field mercury_code
                          , cfci.compatible_mercury_codes_field compatible_mercury_codes
                          , cfci.supported_types_field supported_types
                          , cfci.atc_field
                          , cfci.life_coefficient_field
                        ...
                        LEFT JOIN glpi_suppliers s ON s.id = cfc.suppliers_id_field
                        ...
                        WHERE c.id LIKE '[id]'
                          ...
                          AND ((cfc.tickets_id_out_field IS null AND '[saved_out_ticket_id]' = '%%') OR cfc.tickets_id_out_field LIKE '[saved_out_ticket_id]')
