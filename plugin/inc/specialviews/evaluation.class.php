...
class Evaluation extends View
{
    ...
    protected function getSettings(): array
    {
            ...
            $queries[] = "
					...
					LEFT JOIN ( SELECT tu.users_id user_id, count(t.id) ticket_count
											...
											WHERE NOT t.is_deleted = 1
												...
												AND cft.effective_date_field >= '[start_date]'
												AND cft.effective_date_field <= '[end_date]'
        ...
        return [
            ...
            'show_limit' => Session::haveRight('plugin_iservice_view_evaluation', UPDATE),
            'filters' => [
                'start_date' => [
                    'type' => 'date',
                    ...
                    'link' => [
                        ...
                        'query' => "
												SELECT
													...
													, t.date date_open
													, CASE cft.effective_date_field WHEN '0000-00-00' THEN NULL	ELSE cft.effective_date_field END effective_date
													, u.id tech_park_id
											  ...
											  WHERE cft.effective_date_field >= '[start_date]' AND cft.effective_date_field <= '[end_date]'
