    ...
    'query' => "
        ...
        from (
            select
                ...
                , cft.total2_black_field - coalesce(
                    (
                        select cft2.total2_black_field
                        ...
                        where t2.is_deleted = 0
                          ...
                          and (cft2.effective_date_field > cft.effective_date_field or (cft2.effective_date_field = cft.effective_date_field and t2.id > t.id))
                        order by cft2.effective_date_field, t2.id
                        limit 1
                    ), cft.total2_black_field) black_difference
                , cft.total2_color_filed
                , cft.total2_color_filed - coalesce(
                    (
                        select cft2.total2_color_field
                        from glpi_tickets t2
                        join glpi_items_tickets it2 on it2.tickets_id = t2.id and it2.itemtype = 'Printer'
                        where t2.is_deleted = 0
                          and t2.status = " . Ticket::CLOSED . " 
                          and it2.items_id = it.items_id
                          and (ctf2.effective_date_field > ctf.effective_date_field or (ctf2.effective_date_field = ctf.effective_date_field and t2.id > t.id))
                        order by ctf2.effective_date_field, t2.id
                        limit 1
                ...
                , (select ctf2.effective_date_field from glpi_tickets t2 where t2.id = next_tid) next_data_luc
            from glpi_tickets t
            ...
            join glpi_plugin_fields_ticketticketcustomfields cft2 on cft2.items_id = t2.id and cft2.itemtype = 'Ticket'
            where t.is_deleted = 0
              and t.status = " . Ticket::CLOSED . " 
              and it.items_id > 0
              and ctf.effective_date_field > '$negative_counter_differences_since'
            order by it.items_id, ctf.effective_date_field, t.id
            ) q
