...
return [
    'query' => "
        select t.id tid, cft.total2_black_field, cft.total2_color_field, REPLACE(SUBSTR(l.new_value, POSITION('(' IN l.new_value) + 1), ')', '') pid, p.name
        from glpi_tickets t
        ...
        where t.is_deleted = 0 and coalesce(cft.total2_black_field, 0) + coalesce(cft.total2_color_field, 0) > 0
          and it.id is null
          and (p.id is not null or l.new_value is null)
          and cft.effective_date_field > '2017-03-01 00:00:00'
        group by t.id
        ",
    'test' => [
        'alert' => true,
