...
return [
    'query' => "
        select t.id tid, t.total2_black, t.total2_color, REPLACE(SUBSTR(l.new_value, POSITION('(' IN l.new_value) + 1), ')', '') pid, p.name
        from glpi_tickets t
        ...
        where t.is_deleted = 0 and coalesce(t.total2_black, 0) + coalesce(t.total2_color, 0) > 0
          and it.id is null
          and (p.id is not null or l.new_value is null)
          and t.data_luc > '2017-03-01 00:00:00'
        group by t.id
        ",
    'test' => [
        'alert' => true,
