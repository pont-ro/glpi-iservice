...
return [
    'query' => "
        ...
        where t.is_deleted = 0 and coalesce(t.total2_black_field, 0) + coalesce(t.total2_color_field, 0) > 0
          ...
          and t.effective_date_field > '2024-06-15 00:00:00'
        group by t.id
        ",
    'test' => [
        'alert' => true,
        ...
        'positive_result' => [
            ...
            'iteration_text' => "Ticket <a href='$CFG_PLUGIN_ISERVICE[root_doc]/front/ticket.form.php?id=[tid]' target='_blank'>[tid]</a> has black counter [total2_black_field] and color counter [total2_color_field], but printer <b>[name]</b> is missing from it",
        ],
    ],
];
