...
return [
    'query' => "
        select c.id, cfc.tickets_id_use_field
        from glpi_cartridges c
        join glpi_plugin_fields_cartridgecartridgecustomfields cfc on cfc.items_id = c.id and cfc.itemtype = 'Cartridge'
        left join glpi_tickets t on t.id = cfc.tickets_id_use_field and t.`status` = 6
        where c.date_use is null and cfc.tickets_id_use_field is not null;
        ",
    'test' => [
        'alert' => true,
