...
return [
    'query' => "
        select
            hmf.cod1 as cui_hmarfa
          , cfs.uic_field as cui_iservice
          , s.id
          , s.name from glpi_plugin_fields_suppliersuppliercustomfields cfs
        ...
        where hmf.cod1 <> cfs.uic_field and (hmf.cod1 is not null and hmf.cod1 <> 'null')
        ",
    'test' => [
        ...
        'positive_result' => [
            ...
            'iteration_text' => "Partner <a href='$CFG_GLPI[root_doc]/front/supplier.form.php?id=[id]' target='_blank'>[name]</a> " .
                ...
                "<a href='javascript:void(0);' onclick='ajaxCall(\"$CFG_PLUGIN_ISERVICE[root_doc]/ajax/manageItem.php?itemtype=PluginIserviceSupplier&operation=update&id=[id]&uic_field=[cui_hmarfa]\", \"\", function(message) {if (isNaN(message)) {alert(message);} else {\$(\"#fix-partner-cui-[id]\").html(\"»»» CUI updated in iService with <b>[cui_hmarfa]</b> «««\");}});'>" .
