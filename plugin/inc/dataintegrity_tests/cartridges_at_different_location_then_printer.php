...
return [
    'query' => "
        select c.id cid, cfc.locations_id_field l1id, l1.completename l1_name, p.id pid, p.name pname, p.locations_id l2id, l2.completename l2_name
        ...
        where c.date_use is not null and c.date_out is null
          and coalesce(case cfc.locations_id_field when -1 then 0 else cfc.locations_id_field end, 0) != coalesce(case p.locations_id when -1 then 0 else p.locations_id end, 0)
        ",
    'test' => [
        'alert' => true,
