...
class PrinterCountersV3 extends PluginIserviceViewPrinter
{
    ...
    protected function getSettings(): array
    {
        ...
        $settings['query']               = "
            select
              ...
              , CONVERT(concat( 
                  ...
                  ) USING utf8mb4_unicode) estimate_percentages
              ...
              , CONVERT(concat( 
                  ...
                  ) USING utf8mb4_unicode) days_to_visits
            ...
            from (
                select
                  ...
                  , CONVERT(pbuc.values_detail USING utf8mb4_unicode) calc_uc_bk_values_detail
                  , pbuc.ref calc_uc_bk_ref
                  
                  , pcuc.usage_coefficient calc_uc_c
                  , CONVERT(pcuc.values_detail calc_uc_c_values_detail USING utf8mb4_unicode)
                  , pcuc.ref calc_uc_c_ref
                  
                  , pmuc.usage_coefficient calc_uc_m
                  , CONVERT(pmuc.values_detail calc_uc_m_values_detail USING utf8mb4_unicode)
                  , pmuc.ref calc_uc_m_ref
                    
                  , pyuc.usage_coefficient calc_uc_y
                  , CONVERT(pyuc.values_detail calc_uc_y_values_detail USING utf8mb4_unicode)
                  ...
                  , CONVERT(@uc := if(@consumableType = 'consumable', 1, case cfci.plugin_fields_cartridgeitemtypedropdowns_id
                                                                 ...
                                                                 end) USING utf8mb4_unicode) usage_coefficient
                  , @dba := coalesce(cfp.daily_bk_average_field, 0) dba
                  , @dca := coalesce(cfp.daily_color_average_field, 0) dca
                  , CONVERT(@da := if(cfci.plugin_fields_cartridgeitemtypedropdowns_id in (2, 3, 4), if(@dca = 0, 100, @dca), if(@dba + @dca = 0, 100, @dba + @dca)) USING utf8mb4_unicode) daily_average_counter
                  ...
                  , CONVERT(if (@compatible_printer_count > 0, round(coalesce((@atc * @lc * @uc - (@lastClosedCounter - @installedCounter)) / @da, 180) - datediff(NOW(), @lastClosedDate) + (@atc * @lc * @uc / @da) * (@changeable_count / @compatible_printer_count)), 0) USING utf8mb4_unicode) days_to_visit
        ...
        $settings['columns']['dba'] = [
            'title' => 'Nr. mediu bk',
            'align' => 'center',
            'format' => 'function:\GlpiPlugin\Iservice\Views\PrinterCountersV3::getDailyAverageDisplay("dba", $row);',
        ];
